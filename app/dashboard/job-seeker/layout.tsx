...
export default function JobSeekerDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        setPathCheck(route);
        if (route && route !== '/dashboard/job-seeker') window.location.href = route; // role mismatch to server provided target
      } catch {
  ...
  if (error) {
    if (errorCode === 'NETWORK_ERROR' && loading) {
      return <DashboardLoading variant="job-seeker" onRetry={() => refresh()} onTimeout={() => setTimedOut(true)} />;
    }
    return <DashboardError message={error} code={errorCode} recovery={recovery} onRetry={() => refresh()} />;
  }
  ...
  if (pathCheck === 'error') return <DashboardError message="Failed to validate route access." code="ROUTE_CHECK_FAILED" onRetry={() => refresh()} recovery={recovery} />;
